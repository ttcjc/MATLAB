...
for i = 1:size(times,2)
	...
	switch method
		...
		case 'A'
			...
			particleData.globalPos{times(1,i),1}(:,1) >= min(xRange) & particleData.globalPos{times(1,i),1}(:,1) <= max(xRange) & ...
			particleData.globalPos{times(1,i),1}(:,2) >= min(yRange) & particleData.globalPos{times(1,i),1}(:,2) <= max(yRange) & ...
			particleData.globalPos{times(1,i),1}(:,3) >= min(zRange) & particleData.globalPos{times(1,i),1}(:,3) <= max(zRange));
	...
	for j = 1:size(contaminantData.active{i,1},1)
		xDist = abs(contaminantData.globalPos{i,1}(j,1) - fieldData.x(:,1,1));
		yDist = abs(contaminantData.globalPos{i,1}(j,2) - fieldData.y(1,:,1));
		zDist = abs(contaminantData.globalPos{i,1}(j,3) - fieldData.z(1,1,:));
