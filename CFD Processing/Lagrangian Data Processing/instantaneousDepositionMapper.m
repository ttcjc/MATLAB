%% Instantaneous Lagrangian Deposition Mapper v1.0
...
disp ('====================================');
disp ('Instantaneous Deposition Mapper v1.0');
disp ('====================================');
disp (' ');
...
for i = 1:height(particleData.time)
    ...
    % Collate Newly Deposited Particles
    ...
    switch method
        ...
        case 'A'
            ...
            for j = 1:height(particleProps)
                ...
                contaminantData.cumulative.(prop){i,1} = particleData.(prop){i,1}(index,:);
            end
            ...
            if i == 1
                ...
                for j = 1:height(particleProps)
                    prop = particleProps{j,1};
                    contaminantData.(prop){i,1} = contaminantData.cumulative.(prop){i,1};
                end
                
            else
                dataOld = horzcat(contaminantData.cumulative.origId{(i-1),1}, ...
                                  contaminantData.cumulative.origProcId{(i-1),1});
                dataNew = horzcat(contaminantData.cumulative.origId{i,1}, ...
                                  contaminantData.cumulative.origProcId{i,1});

                [~, index] = setdiff(dataNew, dataOld, 'rows', 'stable');
                
                for j = 1:height(particleProps)
                    prop = particleProps{j,1};
                    contaminantData.(prop){i,1} = contaminantData.cumulative.(prop){i,1}(index,:);
                end
...
if isfield(contaminantData, 'cumulative')
    contaminantData = rmfield(contaminantData, 'cumulative');
end
...
% - Only Instantaneous Impact Sites Loaded in Grid
for i = 1:height(timeInst)
    
    if timeInst(i,1) == 1 % Prevent Plotting of First Time Instance
        disp(['        T = ', num2str(str2double(contaminantData.time{timeInst(i,1),1}), '%.4f'), ...
             ' s - Only Cumulative Data Available (Skipping)']);
        continue;
    elseif isempty(contaminantData.positionCartesian{i,1})
        disp(['        T = ', num2str(str2double(contaminantData.time{timeInst(i,1),1}), '%.4f'), ...
             ' s - No New Particles Recorded on Surface (Skipping)']);
        continue;
    end
...
for i = 1:height(mapData.global.time)
    ...
    switch method
        ...
        case 'A'
            ...
            mapData.global.CoM{i,1}(1,2) = sum(mapData.global.mass{i,1} .* mapData.global.positionGrid(:,2)) / ...
                                           sum(mapData.global.mass{i,1});
            mapData.global.CoM{i,1}(1,3) = sum(mapData.global.mass{i,1} .* mapData.global.positionGrid(:,3)) / ...
                                           sum(mapData.global.mass{i,1});
...
for i = 1:height(mapData.global.time)
    ...
    switch method
        ...
        case 'A'
            ...
            figName = ['Instantaneous_Base_Contamination_Map_T', ...
...
%             figTitle = {'Time (\it{s})', figTime};
            ...
            cLims = [0, 0.065]; % Windsor Full Duration
            xLimsPlot = xLims;
            yLimsPlot = yLims;
            zLimsPlot = zLims;
        ...
        case 'C'
            ...
            fig = figHold;
            figTime = num2str(str2double(mapData.global.time{i,1}), '%.4f');
            figName = ['Instantaneous_Planar_Contamination_Map_T', ...
                       erase(figTime, '.'), '_D', num2str(minD), '_D', num2str(maxD)];
            cMap = viridis(24);
            CoM = mapData.global.CoM{i,1};
...
%             figTitle = {'Time (\it{s})', figTime};
            ...
            cLims = [0, 0.19]; % Windsor Full Duration
            xLimsPlot = xLims;
            yLimsPlot = yLims;
            zLimsPlot = zLims;
...
while ~valid
    ...
    elseif selection == 'y' | selection == 'Y' %#ok<OR2>
        ...
        if ~exist(['/mnt/Processing/Data/Numerical/MATLAB/mapDataInstantaneous/', caseFolder(namePos:end)], 'dir')
            mkdir(['/mnt/Processing/Data/Numerical/MATLAB/mapDataInstantaneous/', caseFolder(namePos:end)]);
        end
        ...
        disp(['    Saving to: /mnt/Processing/Data/Numerical/MATLAB/mapDataInstantaneous/', caseFolder(namePos:end), '/T', startInst, '_T', endInst, '_D', num2str(minD), '_D', num2str(maxD), '_' mapType, '.mat']);
        save(['/mnt/Processing/Data/Numerical/MATLAB/mapDataInstantaneous/', caseFolder(namePos:end), '/T', startInst, '_T', endInst, '_D', num2str(minD), '_D', num2str(maxD), '_' mapType, '.mat'], 'mapData', 'minD', 'maxD', '-v7.3', '-noCompression');
