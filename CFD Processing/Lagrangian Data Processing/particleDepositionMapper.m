...
while ~valid
    disp(' ');
    selection = input('Plot All Time Instances? [y/n]: ', 's');
...
for i = size(times,2):-1:1
    ...
    switch method
        ...
        case 'A'
            index = find(~particleData.active{times(1,i),1} & (particleData.d{times(1,i),1} * 1e6) >= minD & (particleData.d{times(1,i),1} * 1e6) <= maxD & ...
                    round(particleData.positionCartesian{times(1,i),1}(:,1),5) == max(xDims) & particleData.positionCartesian{times(1,i),1}(:,3) > min(zDims));
        ...
        case 'C'
            % Will use 'caseFolder/extractionPlaneData/LagrangianExtractionPlaneData'
    ...
    if isempty(contaminantData.active{i,1})
        ...
        continue
    elseif size(contaminantData.active{i,1},1) < 500
        disp(['    Time = ', num2str(particleData.time{times(i),1}), 's: Insufficient Particles Recorded on Surface for Accurate Interpolation']);
        continue
    end
    ...
    switch method
        ...
        case 'A'
            ...
            mappingData.massNorm{i,1} = mappingData.mass{i,1} / max(mappingData.mass{size(times,2),1});
            ...
            clf(fig);
            set(figure(fig), 'name', ['Base Contamination Map (', num2str(particleData.time{times(i),1}), 's)']);
            ...
            normInst = erase(num2str(str2double(particleData.time{max(times),1}), '%.4f'), '.');
            timeInst = erase(num2str(str2double(particleData.time{times(i),1}), '%.4f'), '.');
            savefig(fig, ['~/MATLAB/Output/Figures/', caseFolder(namePos(end):end), '_Contamination_Map_Base_D', num2str(minD), '_D', num2str(maxD), '_N', normInst, '_T', timeInst]);
            print(fig, ['~/MATLAB/Output/Figures/', caseFolder(namePos(end):end), '_Contamination_Map_Base_D', num2str(minD), '_D', num2str(maxD), '_N', normInst, '_T', timeInst], '-dpng', '-r300');
...
figure(fig);
hold on;
set(figure(fig), 'outerPosition', [25, 75, 800, 800], 'name', 'Colour Bar');
...
axis off;
colormap viridis;
c = colorbar('ticks', tickData, 'location', 'south', 'axisLocation', 'out');
...
% Save Plot
savefig(fig, '~/MATLAB/Output/Figures/Colour_Bar_Contamination_Map');
print(fig, '~/MATLAB/Output/Figures/Colour_Bar_Contamination_Map', '-dpng', '-r300');
