function [ret1, ret2] = streamsliceCJC(varargin)
...
function [vertsout, avertsout] = nicestreams(x,y,u,v,density,arrows)
    ...
    streamoptions = [stepsize min(100000,sum(size(v))*2/stepsize)];
            ...
            for q = 1:2
                ...
                for j=1:size(vv,1)
                    ...
                    if arrows
                        ...
                        if j>1 && cc > 0 && cc <= ncarrow && rr > 0 && rr <= nrarrow && arrowgrid(rr,cc)==0
                            arrowgrid(rr,cc)=1;
                            d = vv(j,:)-vv(j-1,:);
                            d = d/norm(d);
                            if q==2, d = -d; end
