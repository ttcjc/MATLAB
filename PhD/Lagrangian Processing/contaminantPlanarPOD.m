...
varPrime = cell(height(PODdata.time),1);

varInst = PODdata.(PODvar).inst;
varMean = PODdata.(PODvar).mean;
parfor i = 1:height(PODdata.time)
    varPrime{i} = varInst{i} - varMean;
    
    send(dQ, []);
end
clear varInst varMean;

delete(wB);

PODdata.(PODvar).prime = varPrime;
clear varPrime;
